...
export default function History() {
  ...
  async function selectSession(s: Session) {
    ...
    try {
      if (isQualifying(s.session_type)) {
        ...
        const [lb, qd] = await Promise.all([lbRes.json(), qRes.json()])
        setDrivers(lb)
        setQualiData(qd)
      } else {
  ...
  // which drivers to show in results view
  const displayDrivers = isQuali && qualiData
    ? (qualiData[qualiSeg] ?? [])
    : drivers
          ...
          <>
            ...
            {hasResults && (
              <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {displayDrivers.length > 0 && (
                      <p className="text-yellow-500 text-xs font-semibold">
                        {isQuali ? "P" : "🏆"}{isQuali ? "1 " : " "}{displayDrivers[0].driver_abbr} — {displayDrivers[0].team_name}
                      </p>
                    )}
                  </div>
                  <div className="flex gap-1 text-xs">
                    {/* qualifying segment toggles */}
                    {isQuali && qualiData && (
                      <>
                ...
                {view === "results" && <Leaderboard drivers={displayDrivers} />}
